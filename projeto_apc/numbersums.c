...
void adicionaJogador(int *index, char *novoNome, int novaPontuacao)
{
    ...
    for (i = 0; i < *index; i++)
    {
        ...
        {
            // Se o jogador já existe, incrementa a pontuação
            pts[i] += novaPontuacao;
            return; // Sai da função, pois já atualizamos o jogador
        }
    }

    // Se o jogador não existe, adiciona um novo jogador
...
void copiaMatriz(int dificuldade) // clona o arquivo escolhido
{
    ...
    {
    case 1:
        original = fopen("iniciante.txt", "r");
        tamanho = 4;
        break;
    case 2:
        original = fopen("intermediario.txt", "r");
        tamanho = 6;
        break;
    case 3:
        original = fopen("avancado.txt", "r");
        tamanho = 7;
        break;
    default:
        printf(RED "Dificuldade nao selecionada" RESET);
        return;
    }
...
void alteraMatriz(int linha, int coluna, int *vida, int nivel)
{
    ...
    for (i = 0; i < nivel - 1; i++)
    { // pulando o arquivo até o ponto que eu quero
        while (1)
        {
    ...
    char errou, ch;
    for (i = 0; i < (tamanho * (tamanho + 5) + 2); i++)
    { // O cálculo é a forma simplificada de (tamanho+1)*tamanho+tamanho*2*2+2
        getc(matriz);
    }
...
void limpaLinha(int modo, int linha)
{
    ...
    for (int i = 0; i < nivel - 1; i++)
    {
        ...
        while ((ch = fgetc(arquivo)) != EOF)
        {
            ...
            if (ch == '*')
            {          // Avança ao próximo nível ao encontrar '*'
...
void matriz() // Gera a matriz
{
    ...
    printf("\n     ");
    for (x = 0; x < tamanho * 3; x++)
        printf("-"); // A linha divisória é adaptada ao número de colunas
    printf("\n");
...
int main()
{
    ...
    limpaTela();
    char nome[50];
    int num, opcao, modo = 1, vida = 5, limite; // Modo inicia automaticamente no Iniciante
        ...
        {                // JOGAR
            ...
            { // Loop de jogadas
                ...
                while(1)
                {
                    switch (modo)
                    {
                        case 1: limite=4; break;
                        case 2: limite=6; break;
